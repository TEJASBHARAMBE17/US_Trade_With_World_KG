...
class DataLoader:
    ...
    def _load_test_data(tx, country_id, exported):
        ...
        query = "MATCH (year)<-[:tradedYear]-(trade) "
        if exported == "From":
    ...
    def _get_new_years(tx, country_id, exported):
        ...
        query = "MATCH (fta)-[:hasFTA]->(country)"
        if exported == "From":
...
def show_prediction(df, pred, counter_country, fta_year):
    ...
    st.write("")
    st.write(
        f"What if FTA has been in force between USA and {counter_country} since {fta_year}"
    )
    ...
    fig.update_xaxes(showline=True, linewidth=2, gridcolor="Gray")
