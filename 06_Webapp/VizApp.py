...
class VizApp:
    ...
    def get_trade_value(self, country_id, year_from, year_to):
        with self.driver.session(database="neo4j") as session:
            return session.execute_write(
                self._get_trade_value, country_id, year_from, year_to
            )
    ...
    def _get_gdp(tx, country_id, year_from, year_to):
        ...
        query = (
            "MATCH (p:Country) " "WHERE (p.countryID=$country_id OR p.countryID='usa') "
        )
    ...
    def get_top_exported_from(self, country_id, top, year_from, year_to):
        with self.driver.session(database="neo4j") as session:
            result = session.execute_write(
                self._get_top_exported_from, country_id, top, 1, year_from, year_to
            )
            result = pd.concat(
                [
                    result,
                    session.execute_write(
                        self._get_top_exported_from,
                        country_id,
                        top,
                        2,
                        year_from,
            ...
            return result.drop_duplicates().sort_values(
                by="Trade Value", ascending=False
            )
    ...
    def _get_top_exported_from(tx, country_id, top, case_num, year_from, year_to):
        ...
        result = tx.run(
            query, country_id=country_id, top=top, year_from=year_from, year_to=year_to
        )
        try:
            df = pd.DataFrame(
    ...
    def get_top_exported_to(self, country_id, top, year_from, year_to):
        with self.driver.session(database="neo4j") as session:
            result = session.execute_write(
                self._get_top_exported_to, country_id, top, 1, year_from, year_to
            )
            result = pd.concat(
                [
                    result,
                    session.execute_write(
                        self._get_top_exported_to,
                        country_id,
                        top,
                        2,
                        year_from,
    ...
    def get_sections_sum(self, country_id, year_from, year_to):
        with self.driver.session(database="neo4j") as session:
            df = session.execute_write(
                self._get_sections_sum_exported_from, country_id, year_from, year_to
            )
            df = pd.concat(
                [
                    df,
                    session.execute_write(
                        self._get_sections_sum_exported_to,
                        country_id,
                        year_from,
                        year_to,
                    ),
    ...
    def _get_sections_exported_from(tx, country_id, year_from, year_to):
        ...
        try:
            df = pd.DataFrame(
                [
                    [row["year"], row["value"], row["section"], row["name"]]
                    for row in result
                ],
    ...
    def _get_items(tx, product):
        ...
        try:
            return pd.DataFrame(
                [[row["product"], row["category"], row["item"]] for row in result],
...
def run_visualization(country_name, country_id, year_from, year_to, product):
    ...
    nodes = (
        list(df_graph["Product"].unique())
        + list(df_graph["Category"].unique())
        + list(df_graph["Item"].unique())
    )
    edges = list(
        df_graph[["Product", "Category"]].drop_duplicates().to_records(index=False)
    ) + list(df_graph[["Category", "Item"]].drop_duplicates().to_records(index=False))
